...
**学习建议：** 这章可以按真实项目的优先级读：流式解决用户等待时看不见进度的问题，持久化解决中断和恢复的问题，时间回溯更偏调试，子图更偏复用和拆分。每学一个特性，都先问它在项目里解决哪类痛点，再看 API；这样高级特性不会变成“会用但不知道何时用”的清单。
...
1. Streaming、Persistence、Time-Travel、Subgraph 这几个能力分别解决哪类真实痛点？
   ...
   **参考思路：** Streaming 解决过程不可见，Persistence 解决中断恢复，Time-Travel 解决回放调试，Subgraph 解决复杂流程拆分和复用。先从痛点理解，再看 API。
