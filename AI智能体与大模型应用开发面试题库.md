...
如果先给一句定义，我会说：AI Agent 是一种以大语言模型为推理核心，能够围绕目标持续感知、规划、调用工具并执行动作的智能系统。
...
我一般不会把 Prompt 优化理解成“反复手改几句提示词”，而是把它当成一个小型工程闭环。真正想把准确率稳定做上去，核心不是玄学技巧，而是 **任务拆解、样例驱动、评测回归和边界收敛**。
...
如果面试里要说得更务实，我通常会补一句：所谓“准确率 > 95%”一定要绑定具体任务，比如分类、抽取、格式生成、客服问答，它不是一个通用承诺值。真正的工程目标应该是让某个具体任务在固定评测集上稳定达标，而不是追一个脱离场景的数字。
...
我会先强调，这题和“模型路由”不是一回事。模型路由更像“这次该用大模型还是小模型”，而意图识别 / 任务路由更像“这次请求到底该走 FAQ、RAG、Tool、Agent、Workflow，还是直接转人工”。
...
工程上我一般不会做“单层万能路由器”，而是更偏好多级路由：

- 先做业务意图路由：判断这是 FAQ、RAG、Tool 还是 Agent。
- 再做子域路由：比如客服里再分订单、退款、工单、投诉。
- 最后再做模型路由：在具体链路里决定用强模型还是轻模型。

这样做的好处是更容易治理，也更容易评测。评估时我会重点看：路由准确率、低置信度占比、误路由成本、澄清命中率，以及每条路由分支的最终任务完成率。
...
如果一句话总结，就是：内网方案不是“离线版 Demo”，而是一套更强调安全边界、运维治理和模型可替换性的生产架构。
...
Code Agent 的核心不是“会写几行代码”，而是能围绕一个代码任务完成 **读仓库、定计划、改文件、跑验证、根据结果继续修正** 的闭环。
...
所以一个成熟的 Code Agent，更像“面向代码任务的执行系统”，而不是“代码自动补全工具放大版”。

**常见追问：**

- 千万行仓库里，Code Agent 最容易先卡在哪里？
