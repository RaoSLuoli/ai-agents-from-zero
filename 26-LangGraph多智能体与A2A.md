...
也就是说：**A2A 关心的是“Agent 和 Agent 怎么协作”。**
...
如果主管把子 Agent 当工具调用，可以这样理解：用户始终主要和主管交互，所有路由都由主管决定，子 Agent 更像“高级工具”。这类模式的好处是：上下文隔离更强，统一控制更强，工程边界更清楚。代价通常是：主管压力更大，有时多一层调用开销。
...
也就是说：Supervisor 的重心是“中心调度”；Handoff 的重心是“控制权转移”。
...
从第 24 章学过的控制原语角度看，Handoff 往往会用到 `Command` 和 `Send`：

- `Command` 用来表达“当前 Agent 处理完后，控制权交给哪个 Agent，并同时更新父图状态”。
- `Send` 可以把“目标 Agent + 传给它的局部状态”打包成一次运行时跳转。
- `Command.PARENT` 这类用法则说明：子 Agent 或工具不只是返回文本，还可以影响父图的下一步控制流。
...
例如，一个代码助手需要会代码审查、测试修复、提交信息生成和文档改写。这些能力可以先做成 Skills，而不一定要拆成 4 个 Agent。
