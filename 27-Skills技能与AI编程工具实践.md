...
**学习建议：** **Skill 是给 Agent 的可复用说明书**。它不是神秘能力，也不等于独立智能体。它解决的是另一类很实际的问题：同一类任务反复提示、规则散落、主提示词越来越长、多个 Agent 难以共享同一套经验。
...
换句话说，先让模型知道“我会什么”，任务匹配后再展开“具体怎么做”，只有真正需要时才读取脚本、模板和资料。
...
写完 Skill 后，不要只看文件是否存在。更重要的是测试它能不能在合适场景触发。
...
> **图 27-3：** Skill 位于“提示词工程”和“Agent 工程”之间。它不直接替代 Tool、MCP 或 Agent，而是把任务方法和上下文资源整理成可复用单元。
...
### 7.3 Claude Code 中怎么使用 Agent Skills
...
### 7.4 DeepAgents / LangChain 中怎么理解 Skills
...
1. Skill 和普通 Prompt 最大的区别是什么？

   **答案：** Prompt 通常是一次性的当前任务指令，Skill 是可复用、可发现、可按需加载的能力包，适合沉淀重复工作流、专业流程、模板、脚本和资源。

2. 为什么 `description` 对 Skill 特别重要？

   **答案：** 因为模型通常先读取 `name` 和 `description` 来判断是否使用某个 Skill。描述太泛会误触发，描述太模糊会触发不了。好的描述应该同时说明“做什么”和“什么时候用”。

3. Skill、Tool、Subagent 分别适合解决什么问题？

   **答案：** Skill 适合封装任务方法和上下文资源；Tool 适合执行明确动作；Subagent 适合独立处理一类复杂子任务，并拥有自己的上下文、工具和职责边界。

4. Cursor Rules 和 Agent Skills 是不是一回事？

   **答案：** 不是。Cursor Rules 更偏常驻或按路径附加的项目规则和系统级指导；Agent Skills 更偏按任务触发的能力包。两者都能复用经验，但加载方式和适用场景不同。

5. 什么情况下应该优先写 Skill，而不是拆一个新的 Agent？

   **答案：** 如果只是同一个 Agent 需要掌握一套专业流程、输出格式或参考资料，优先写 Skill。只有当任务需要独立角色、独立上下文、独立工具权限和复杂协作时，才更适合拆成新的 Agent 或 Subagent。
